...
sisältäisivätkin natiivikomponentteja. Vaikka sovelluksen ohjelmoisi
...
metodeita ja käyttää sen tietorakenteita. Tässä yhteydessä kielten
luokittelu matalan ja korkean tason kieliin ei perustu kielen
abstraktiomekanismien ilmaisuvoimaan vaan kielen toteutustapaan: Java
on korkean tason kieli, sillä Java-ohjelmia suoritetaan välikielen
muodossa abstraktissa virtuaalikoneessa eikä suoraan laitteistotason
...
toimia eri virtuaalikoneiden kanssa ilman uudelleenkääntämistä.  Tämän
vuoksi JNI ei voi tehdä mitään oletuksia virtuaalikoneen sisäisestä
toteutuksesta @@jnibook(145). Periaate on tärkeä ennen kaikkea siksi,
että virtuaalikoneen on aina kyettävä järjestämään uudelleen omia
tietorakenteitaan esimerkiksi automaattisen muistinhallinnan,
JIT-kääntämisen ja erilaisten optimointien toteuttamiseksi. JNI:tä
edeltänyt varhainen Javan natiivirajapintatoteutus oli
suoraviivaisempi ja asettikin rajoituksia virtuaalikoneen
roskienkeruualgoritmeille.
...
JNI:n tarjoaman rajapinnan ylitse.
...
mutta JNI sallii virtuaalikoneen myös /kopioida/ kohteen arvon
...
viite ~HelloWorld~-olioon -- kyseessä on ilmentymämetodi, joka saattaa
käyttää ilmentymän palveluita. Luokkametodin tapauksessa
...
~int~.  ~JNIEXPORT~- ja ~JNICALL~ -makrot määrittävät
natiivimetoditoteutusten sekä JNI-funktioiden kutsu- ja
linkityskäytännöt @@jnibook(170).
...
suoritusympäristö eroavat kehitysympäristön
...
C-kieliset tyyppinimet ovat peräisin ~jni.h~
-otsaketiedostosta. Primitiiviparametrien arvot välitetään
...
Ohjelmointityössä ehkä yleisimpien tietotyypien, merkkijonojen ja
...
paljastamista -- ainakin dokumentaation tasolla.
...
eksplisiittistä hallinta. Natiivirajapinta tarjoaa
...
voidaan vapauttaa. Oletuksena kaikki muut oliviitteet, jotka JNI antaa
...
päättymiseen. Niiden käyttö ei ole säieturvallista. Globaalit viitteet
on eksplisiittisesti luotava ~NewGlobalRef~ -kutsulla; ne estävät
olion roskienkeruun ja mahdollistavat olioihin viittaamisen yli eri
natiivimetodien kutsujen myös eri säikeistä.
...
siirtää ohjelman suorituksen pitkäksikin aikaa syvälle
natiivikomponenttiin, joten kyseisen metodin kutsukaari saattaa pitää
...
Olen ennakoinut, että natiivirajapinnan ylittäminen voi lisätä
ylimääräisiä suorituskykyrasitteita aivan tavallisiin
laskentatehtäviin kuten merkkijonojen ja taulukoiden käsittelyyn,
olioiden kenttien ja metodien käyttämiseen sekä natiivimetodien
