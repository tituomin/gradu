...
arvoina Java-kielessä. Eräs tapa säilyttää Java-ohjelmassa viite
natiivitietorakenteeseen on sen osoitteen tallentaminen
~long~-tyyppiseen kenttään; osoitteen hyödyntäminen edellyttää lopulta
sen välittämistä C-koodille joka muuntaa sen osoitintyyppiseksi
arvoksi @@jnibook(123--130).
...
Java-ohjelman tietorakenteita pääsemättä kuitenkaan käsiksi näiden
rakenteiden sisäiseen toteutukseen tai käsittelemään suoraan
