...
kutsua kohti eli karkeasti arvioidden yli 20 sekuntia 512-alkioisen
...
se, että vasteajatkin ovat yhtäsuuret \path{Get\tau{}ArrayElements}-
ja \path{GetStringChars}-kutsuissa. Keskeisimmät rasitteet syntyvät
siis epäsuoran globaalin viitteen purusta ja taulukon
kiinnikytkemisestä.

Kopioivien operaatioiden vasteajat noudattelevat melko suoraan
kopioitavan primitiivityypin kokoa. Operaatioiden vasteajoissa on
vakioelementti, joka koostuu taulukkoviitteen purusta ---
kiinnikytkemistä ei kopioitaessa tarvita. Lineaarisen riippuvuuden
kulmakerroin määräytyy melko suoraan itse kopiointioperaatiosta
(\path{memcpy}), määräävänä tekijänä kopioitavien elementtien koko
tavuina. Siten ~double~-taulukon kopiointi (kun vakiorasite on
vähennetty) on noin kaksi kertaa niin raskasta kuin ~float~-taulukon.
Myös \path{GetStringRegion} -operaation rasite vastaa hyvin
16-bittisen char-taulukon kopioimisen rasitetta. Kutsuprofiileissa
tämä näkyy niin, että ~memcpy~-operaation prosenttiosuus operaatiossa
kasvaa tasaisesti.
