...
viitetaulukkoa lukemalla roskienkeruu pysyy tietoisena JNI-rajapinnan
yli välitetyistä olioviitteistä).
...
ajan --- nimittäin itse kutsun kohteena olevan aliohjelman
suorittamiseen kuluvan ajan.
...
lisärasitteeseen ainakin osittainen selitys seuraavista
huomioista. {{{jc}}}-profiilista puuttuu
\path{ScopedJniThreadState}-olion luominen ja
...
lokaaleja. Kohdassa \ding{202} {{{TODO}}} ei {{{jc}}} -suunnassa tarvita rinnakkaisten
...
yhteisen benchmarkin alle. Benchmarkeille on annettu nimet seuraavasti:
...
on kutsuttava sen /lisäksi/ että merkkijonoa varsinaisesti käsitellään
...
(2) kutsu, jossa Java-komponentille lähetetään viite kyseiseen
olioon.
...
osoitteen saaminen merkkijonon muistialueelle. Listauksessa
\ref{profile:C2JReadUnicode-512} näkyy suurimpana vasteajan
komponenttina jälleen globaalin viitteen purku, koska käytetty
~jstring~-viite on globaali. Tämän jälkeen merkittävä kustannus
(8.44%) syntyy jälleen muistinhallintaan liittyvästä toimesta, eli
merkkijonon sisäisen merkkitaulukon sulkemisesta roskienkeruun
ulkopuolelle (\path{pinPrimitiveArray}). Itse merkkijonon merkkien
...
muistialuetta jota voi käsitellä tehokkaasti molemmissa ympäristöissä.
