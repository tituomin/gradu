...
näy. Kun staattista metodia kutsutaan, välitetään JNI-rajapinnan yli
\path{jclass}-viite, mutta Dalvikissa ei tällaisesta luokkaviitteestä siis
pidetä kirjaa viitetaulukon avulla. Rasite on 500 000 kierroksella
1.49 sekuntia, kun vastaavalla dynaamisella kutsulla se on 2.68
sekuntia, mistä suurin osa selittyy juuri kutsujen
...
vapauttaa, jotta se ei loppuisi. Kutsuprofiileja voi toki rajallisesti
hyödyntää tutkimalla siitä roskienkeruun dominoiman osuuden
alapuolelle jääviä osia.
...
Seuraavaksi luomme mittaustulosten perusteella karkean mallin, joka
kuvaa, mistä osatekijöistä JNI-rajapinnan käytön rasitteet syntyvät.
Mallin lukuarvot ovat suhdelukuja, jotka kertovat eri rasitteiden
suuruusluokan suhteessa toisiinsa.
...
|---+------------------------------+----------------------------------+------------+------------|
| 1 | virtuaalikoneen tilamuutos   | kutsujen määrä                   | 123 n      | 84 n       |
|---+------------------------------+----------------------------------+------------+------------|
| 2 | metodikutsun perusrasite     | kutsujen määrä                   | 50 n       | 30 n       |
| 3 | parametrien välitys          | primitiiviparametrien määrä      | 2 n        | 3 n        |
|---+------------------------------+----------------------------------+------------+------------|
| 4 | viiteparametrit              | instanssi- vai staattinen metodi | 5 tai 0    | 5 tai 2    |
| 5 |                              | globaalit viitteet               | 70 n       | 60 n       |
| 6 |                              | lokaalit viitteet                | 30 n       | 20 n       |
|---+------------------------------+----------------------------------+------------+------------|
| 7 | muistialueen kiinnitys       |                                  | 20 n       | -          |
|---+------------------------------+----------------------------------+------------+------------|
| 8 | datan kopiointi              | tavujen määrä                    | 400 n      | -          |
|---+------------------------------+----------------------------------+------------+------------|
#+LATEX: }
...
JNI:n välityksellä tehtäviin kutsuihin tulee toisaalta myös merkittävä
lisärasite, joka johtuu kutsun epäsuoruudesta, kutsukonvention
muuntamisesta kielten välillä, mahdollisten välttämättömien viitteiden
käsittelystä sekä edellä mainitusta tilamuutoksesta.
...
Mainittuja ongelmakohtia pystyy välttämään esimerkiksi seuraavassa
esimerkissä hahmotellulla tavalla, jossa Javalla allokoitujen olioiden
käyttö vältetään ~java.nio.DirectByteBuffer~ -olion avulla. Kyseinen
olio välitetään komponentista toiseen kertaluontoisesti, sen voi
allokoida suoraan natiivikomponentissa, ja sen lukeminen
Java-komponentissa on tehokasta kopioivien ~bulk~-operaatioiden
avulla.

* Esimerkki datan siirtämisen kustannuksista
...
metodeista, ja osa mittaustuloksistakin jätettiin lopulta
...
Sen sijaan potentiaalisesti oleellisemmat, mutta yksinkertaisuuden
vuoksi pois rajatut ja todennäköisesti vaikeasti mitattavat palvelut
koskevat rinnakkaisohjelmointia (\path{MonitorEnter} ja
\path{MonitorExit}) ja poikkeusten käsittelyä. Ne olisivat
...
jos sitä voi välttää -- viitetyyppisten olioiden käytöstä kun syntyvät
