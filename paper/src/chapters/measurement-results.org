...
vasteajoilla mittaushäiriöt ovat jo suhteellisesti
...
taustaprosesseista aiheutuva rasite (28.36%), minkä takia
kokonaisuoritusaika on 71.64% eikä 100%.
...
profiilista erottuu viitteiden välittämisen aikarasite. Avain on
...
parametrinä JNI-rajapinnan yli, eli juuri JNI-koodin \path{jobject *}
...
ovat /globaaleja/ viitteitä, joita säilytetään Dalvikin globaalissa
...
sattumalta hitain suorituspolku \path{dvmDecodeIndirectRef} -metodin
...
Edelleen C \to Java -kutsusuunnassa ja kahdellakymmenellä
~int~-parametrilla saadaan listauksen \ref{src:C2JBenchmark00146}
kutsuprofiili.

# {{{profile(C2JBenchmark00146)}}}

Profiilissa näkyy edelleen \path{dvmDecodeIndirectRef} (\ding{202})
13.42% suoritusajalla --- benchmarkissa kutsutaan Java-olion metodia,
joten olio täytyy välittää viitteenä JNI-kutsussa.
...
~THREAD_NATIVE~, jonka aikana roskienkeruu on mahdollista. Suorituksen
ollessa käynnissä tavallisessa Java-tilassa, tila on ~THREAD_RUNNING~,
jolloin roskienkeruu edellyttää tilamuutosta esimerkiksi
...
mitattu kattavasti staattisia metodikutsuja vaihtelevilla
...
parhaimmillaankin (staattinen metodi parametrimäärällä 0) 1.76-kertaa
{{{jc}}}-kutsusuunnan vasteaika? Luvussa täytyy huomioida, että
mitatut kokonaisvasteajat sisältävät muutakin kuin tarkastelun
kohteena olevassa JNI-toteutuksessa vietetyn ajan --- nimittäin itse
kutsun kohteena olevan aliohjelman suorittamiseen kuluvan ajan.
...
lisätty yksinkertainen sivuvaikutuksellinen operaatio, jotta sitä ei
optimoitaisi tyhjänä pois käännösvaiheessa.

JNI-lisärasitteen suuruudesta /C/ saa paremman arvion seuraavasti.
{{{cj}}}-suunnan rasite saadaan vähentämällä ({{{cj}}})-vasteajasta
({{{jj}}})-vasteaika. Sama pätee myös toiseen suuntaan. Olettaen, että
itse lähtösilmukan yleisrasite on merkityksettömän pieni suhteessa
koko vasteaikaan, saadaan näin selville, miten paljon kauemmin kestää
kutsua samaa kohdealiohjelmaa JNI:n yli verrattuna normaaliin kutsuun
ilman JNI:tä. Tällä tavalla saadaan tarkennettua {{{cj}}} kutsusuunnan
JNI-rasite tässä parhaimmassa tapauksessa noin 1.19 kertaiseksi
{{{jc}}}-suunnan rasitteeseen nähden.

Listauksien \ref{profile:C2JBenchmark00001} ja
\ref{profile:J2CBenchmark00001} profiileja tutkimalla saadaan tähän
ainakin osaselitystä seuraavista huomioista. {{{jc}}}-profiilista
puuttuu \path{ScopedJniThreadState}-olion luominen ja
\path{dvmChangeStatus}-metodia kutsutaan suoraan. Samoin
\path{callPrep}-funktio puuttuu. Näiden toteutusyksityiskohtien
tarkempi analyysi jätetään tässä tutkielmassa tekemättä.
...
lokaaleja. Kohdassa \ding{202} {{{TODO}}} ei tarvita rinnakkaisten
säikeiden poissulkemista.

{{{profile(J2CBenchmark00206)}}}

** jni-totetutus: kommentteja :noexport:
