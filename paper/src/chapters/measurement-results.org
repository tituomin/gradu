...
Jos taas puskuri on luotu Java-komponentissa, sen edustaman
...
operaatiot ovat edellytyksiä puskurin käytölle, niitä mitattiin
yhdessä ja vasteajaksi saatiin 2.31 sekuntia.


| JNI-kutsu                       | kierrosmäärä | vasteaika | vertailuluku |
|---------------------------------+--------------+-----------+--------------|
| \path{GetDirectBufferAddress} + | 500 000      |           |              |
| \path{GetDirectBufferCapacity}  |              |           |              |
|---------------------------------+--------------+-----------+--------------|
| \path{NewDirectByteBuffer}      | 300          |           |              |
