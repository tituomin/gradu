...
Kuvassa \ref{fig:sequence_java_to_java} \path{JavaCounterparts}-luokka sisältää kaikki tyhjät metodit
...
Java-tynkämetodia.\ref{fig:sequence_c_to_java}
...
Eri permutaatioiden sekvenssikaavioista voi huomata, että mitattava
ohjelmakoodi vaihtelee sen mukaan, onko ~runInternal~ -metodi ja
lähtösilmukka Java- vai natiivikomponentissa. Kun se on
...
lähtösilmukasta suoritustasolla ovat, ei niiden suoritusaikojakaan
...
jossa suoraan silmukan sisälle lisätään lineaarisesti varioiden /n/
...
operaatioita paikallisilla muuttujilla. Interpoloimalla saaduista
vasteajoista kun /n/:n arvoa varioidaan, saadaan arvioitua, mikä
mittausmenetelmän pohjavasteaika on n:n arvolla 0. Näin saadaan
...
JIT-kääntäjä saattaa myös optimoida pois tyhjän silmukan, jolla
ei ole sivuvaikutuksia (aiheesta lisää kohdassa XXXXX).
