...
aikaväli @@ferrari(463), @@advanced-unix(20).[fn:bar]
UNIX-käyttöjärjestelmissä tätä mittaa järjestelmäkutsu =gettimeofday=
...
kriittisiä. Toinen tosiajan ongelma on, moniajojärjestelmistä se on
riippuvainen järjestelmän kokonaistilasta @@ferrari(455); tässä
tutkielmassa muiden prosessien vaikutus tulkitaan tilastolliseksi
varianssiksi.
...
aktiivinen suoritusaika @@advanced-unix(20). Tutkielmassa oletetaan,
...
tosiaikakellon aiheuttaman keskeytyksen laukaisemana, ja näytteeseen
...
yleisimmät pituudet ja jopa sisällöt @@spe(334). Pelkällä kääntäjän
lisäämällä ei-sovelluskohtaisella instrumentaatiolla saadaan myös
tietoa, jota ei muilla tekniikoilla tavoita: jokaisen aliohjelman
kutsujen määrä ja jokaisen kutsun vasteaika.
...
# todo edellisellä rivillä here here
...
abstraktiotason. Käytännössä yhden yksittäisen kutsuja aiheuttavan
kohdan muuttaminen korkeammalla kutsupinossa voi siirtää matalamman
tason suorituskriittiset kohdat aivan muualle.
# dunlavey
...
/eksklusiivinen/ suoritusaika, usein myös /self time/.
# gprof, oprofile, etc.
...
suhtautua pienellä varauksella. Korkealla kutsupinossa olevien
...
asymptoottisen käyttäytymisen tuntemisesta on rajatussa mielessä
hyötyä suorituskyvyn mallintamisessa. Tällainen analyysi tukee
kuitenkin lähinnä yksittäisten algoritmien ja tietorakenteiden
valintaa ja antaa vain rajallisesti tukea järjestelmän
kokonaisarkkitehtuurin laatimiselle. Käytännön ohjelmistokehitykselle
merkittävä puute on lisäksi, että asymptoottinen analyysi pelkistää
ohjelman käyttäytymistä liiaksi jättäen huomiotta yksittäisten
operaatioiden todelliset kustannukset todellisilla laitteilla --
analyysin perusteella tehokas algoritmi saattaa esimerkiksi sopia
huonosti yhteen nykyaikaisen suorittimen välimuistin käyttäytymisen
